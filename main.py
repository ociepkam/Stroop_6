...
VISUAL_OFFSET = 50
FIGURES_SCALE = 0.4
RESULTS = [['EXP', 'TRIAL_TYPE', 'TEXT', 'COLOR', 'WAIT', 'RESPTIME', 'RT', 'TRUE_KEY', 'ANSWER', 'CORR', "MAIN_TIME"]]
...
def show_info(win, file_name, insert=''):
    ...
    msg.draw()
    win.flip()
    key = event.waitKeys(keyList=['f7', 'return', 'space', 's'])
    if key == ['f7']:
...
for idx, block in enumerate(training_trials):
    ...
    for trial in block:
        # prepare trial
        true_key = KEYS[trial['color']]
        reaction_time = data['Training_Resp_time']
        ...
        RESULTS.append(
            ['training', trial['trial_type'], trial['text'], trial['color'], data['Training_Wait_time'],
             data['Training_Resp_time'], reaction_time, true_key, ans, ans == true_key, t])
        check_exit()
        ...
        # wait
        time.sleep(data['Training_Wait_time'] + data['Training_Resp_time'] - reaction_time)
        check_exit()
...
for idx, block in enumerate(blocks):
    for trial in block:
        ...
        # true_key, reaction_time, triggers = prepare_trial_info(trial)
        true_key = KEYS[trial['color']]
        reaction_time = data['Experiment_Resp_time']
        ...
        RESULTS.append(
            ['experiment', trial['trial_type'], trial['text'], trial['color'], data['Experiment_Wait_time']+jitter,
             data['Experiment_Resp_time'], reaction_time, true_key, ans, ans == true_key, t])
        check_exit()
        ...
        # wait
        time.sleep(data['Experiment_Wait_time'] + data['Experiment_Resp_time'] - reaction_time)
        check_exit()
